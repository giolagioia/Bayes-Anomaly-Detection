...
Since withdrawals and deposits represent mutually exclusive outcomes of the same transaction, they were merged into a single monetary variable to obtain a consistent representation of financial flow.

No observations were removed during preprocessing; instead, only non-informative or redundant variables were excluded. This ensures that the original transactional structure of the dataset is preserved without introducing selection bias.

Moreover, the \textit{time index} is transformed using a logarithmic scale to reduce skewness in transaction timing and to compress large temporal ranges. This improves numerical stability and allows the model to capture nonlinear temporal effects more effectively.
...
The dataset contains in total $116201$ transactions.
...
This produces a binary indicator used in the Bayesian models. The Isolation Forest procedure identifies $5810$ anomalies, corresponding to an anomaly rate of $4.99996\%$. Anomalies are defined using the 95th percentile of the Isolation Forest score distribution, where observations with a score above the 95\% quantile are classified as anomalous (threshold = 0.472158).
...
From a predictive perspective, the Bayesian logistic regression outperforms the frequentist logistic regression in ROC-AUC, log loss, and F1-score when both use a fixed threshold of $0.5$.

The two Bayesian rows in Table 7 refer to the same fitted model. The only difference is the classification threshold. With the standard threshold $0.5$, the model is conservative: it has high precision ($0.8403$) but low recall ($0.2774$). With the ROC-optimal threshold $0.1179$, more observations are classified as anomalies, increasing recall to $0.7874$ and F1-score to $0.5329$, but reducing precision to $0.4027$.
